...
describe('create new project', () => {
    it('when call create a new project with correct parameters, then create them', async () => {
        const response = await request(server.app).post('/project')
            .send({ name: 'ProjectName', issueTracking: 'Trello', backlogList: ['BACKLOG'], workingList: ['ANALYSIS', 'DOING', 'QA', 'Review'], waitList: ['READY TODO', 'READY FOR QA'], doneList: ['DONE'], apiKey: 'apiKey', apiToken: 'apiToken', boardId: 'boardId' })
    ...
    it('given exists project with the same name when call create a new project then return error', async () => {
        ...
        const response = await request(server.app).post('/project')
            .send({
                name: 'projectName', issueTracking: 'Trello', backlogList: ['BACKLOG'], workingList: ['ANALYSIS', 'DOING', 'QA', 'Review'], doneList: ['DONE'], apiKey: 'apiKey', apiToken: 'apiToken', boardId: 'boardId' })
            .set('Accept', 'application/json')
